...
/* But C expects to pass nx, ny, and load by VALUE, which is a mismatch */
extern void compute_displacement(const char *material, long nx, long ny, 
...
double run_solver(char *material, long nx, long ny, double load) {
    ...
    compute_displacement(material, nx, ny, load, &result);
