...
def _get_type_size(t) -> int:
    if isinstance(t, ScalarType):
        if t.is_pointer:
            return 8
        if t.base == "complex":
            return 2 * t.kind_bytes
        return t.kind_bytes
    elif isinstance(t, StructType):
        return t.size
    elif isinstance(t, ArrayType):
        elem_s = _get_type_size(t.element)
        num_el = 1
        for dim in t.shape:
            if dim is not None:
                num_el *= dim
...
def _get_fortran_type_str(t) -> str:
    if isinstance(t, ScalarType):
        if t.base == "real":
...
class Comparator:
    def __init__(self):
        ...
        self.current_f_proc = None

    def _add_mismatch(self, category: str, severity: Severity, msg: str, proc_name: str, ft: str="", ct: str="", file_path: str=None, line_number: int=None):
        if file_path is None:
            file_path = self.current_f_proc.source_file if self.current_f_proc else ""
        if line_number is None:
            line_number = self.current_f_proc.source_line if self.current_f_proc else 0
        self.mismatches.append(Mismatch(category, severity, msg, proc_name, ft, ct, file_path, line_number))
    ...
    def _compare_scalar(self, proc_name: str, param_name: str, ft: ScalarType, ct: ScalarType):
        ...
        if ft.base != ct.base:
            if (ft.base == "character" and ct.base == "integer" and ct.kind_bytes == 1) or \
               (ct.base == "character" and ft.base == "integer" and ft.kind_bytes == 1):
                ...
                self._add_mismatch("BOOL_VS_INT_RETURN", Severity.WARNING, "Bool vs int return", proc_name)
            else:
                self._add_mismatch("SCALAR_TYPE_MISMATCH", Severity.ERROR,
        ...
        if ft.is_value and ct.is_pointer and not ft.is_pointer:
            self._add_mismatch("VALUE_REFERENCE_MISMATCH", Severity.ERROR,
        ...
        if ft.is_pointer and not ct.is_pointer:
            self._add_mismatch("POINTER_DEPTH_MISMATCH", Severity.ERROR,
        ...
        if ft.is_pointer and ct.is_pointer and ft.kind_bytes != ct.kind_bytes:
            ...
            self.mismatches = [m for m in self.mismatches if not (m.category == "SCALAR_SIZE_MISMATCH" and m.proc_name == proc_name)]
                               
        if getattr(ft, 'iso_name', None) == "c_funptr" and (not ct.is_pointer or ct.base != "integer"):
            self._add_mismatch("PLATFORM_FUNPTR_ALIGN", Severity.WARNING, "c_funptr must map to C function pointer", proc_name)
            
        if hasattr(ft, 'is_optional') and ft.is_optional:
            self._add_mismatch("OPTIONAL_NULL", Severity.WARNING,
                               f"Parameter '{param_name}' is OPTIONAL in Fortran. C must check for NULL.",
                               proc_name)
                               
        if getattr(ft, 'iso_name', None) == "c_long":
            self._add_mismatch("PLATFORM_DEPENDENT", Severity.WARNING, "c_long is platform dependent", proc_name)
            
        if getattr(ft, 'iso_name', None) == "c_long_double":
            self._add_mismatch("LONG_DOUBLE_PORTABILITY", Severity.WARNING, "REAL(c_long_double) is non-portable", proc_name)
    ...
    def _compare_types(self, proc_name: str, param_name: str, ft: AnyType, ct: AnyType):
        if isinstance(ft, ScalarType) and isinstance(ct, ScalarType):
            ...
            if ft.rank >= 2 or ct.rank >= 2:
                self._add_mismatch("COLUMN_ROW_MAJOR", Severity.WARNING, "Fortran 2D array is column-major, C is row-major", proc_name)
            ...
            if ct.is_pointer:
                if getattr(ft, 'is_assumed_shape', False):
                    self._add_mismatch("ARRAY_DESCRIPTOR", Severity.ERROR, "Fortran assumed-shape passes CFI_cdesc_t", proc_name)
                elif ft.element.base == "character":
                    self._add_mismatch("CHAR_NUL_TERMINATION", Severity.WARNING, "Fortran strings are not NUL terminated", proc_name)
                ...
                self._compare_types(proc_name, f"{param_name}[]", ft.element, ct)
                 ...
                 if ft.rank >= 2:
                     ...
                     self._add_mismatch("ARRAY_RANK_MISMATCH", Severity.ERROR,
            ...
            if not ft.is_bind_c:
                self._add_mismatch("NON_INTEROPERABLE_TYPE", Severity.ERROR,
                ...
                for fname, ftype in ft.fields:
                    if isinstance(ftype, ScalarType) and ftype.base == "logical":
                        self._add_mismatch("LOGICAL_IN_NON_BIND_TYPE", Severity.ERROR,
                ...
                for fname, ftype in ft.fields:
                    if isinstance(ftype, StructType) and not ftype.is_bind_c:
                        self._add_mismatch("NESTED_NON_BIND", Severity.ERROR,
            ...
            if has_pragma_pack:
                self._add_mismatch("PACKED_STRUCT", Severity.WARNING,
            ...
            if len(ft.fields) != len(ct.fields):
                self._add_mismatch("STRUCT_FIELD_COUNT_MISMATCH", Severity.ERROR,
                    ...
                    for j in range(len(ft.fields)):
                        if i != j:
                            ...
                            if f_fname == c_fname2 and c_fname == f_fname2:
                                swapped = True
                                self._add_mismatch("FIELD_ORDER", Severity.ERROR,
                                    f"FIELD_ORDER: Field[{i}] Fortran='{f_fname}'({_get_fortran_type_str(f_ftype)}), C='{c_fname}'({_get_c_type_str(c_ftype)}).\n"
                                    f"               Field[{j}] Fortran='{f_fname2}'({_get_fortran_type_str(f_ftype2)}), C='{c_fname2}'({_get_c_type_str(c_ftype2)}).\n"
            ...
            if ft.is_value:
                self._add_mismatch("COMPLEX_STRUCT_ABI", Severity.WARNING, "Complex passed as struct", proc_name)
            else:
                self._add_mismatch("COMPLEX_ABI_MISMATCH", Severity.ERROR,
            ...
            if ct is None:
                self._add_mismatch("PLATFORM_FUNPTR_ALIGN", Severity.WARNING, "c_funptr alignment trap on Apple M1", proc_name)
            else:
                self._add_mismatch("FUNPTR_VS_PTR", Severity.ERROR, "c_funptr must map to C function pointer", proc_name)
        else:
            self._add_mismatch("TYPE_CATEGORY_MISMATCH", Severity.ERROR,
    ...
    def compare(self, f_procs: List[InterfaceProc], c_procs: List[InterfaceProc]) -> List[Mismatch]:
        ...
        for name, f_proc in f_proc_map.items():
            self.current_f_proc = f_proc
            if name not in c_proc_map:
                if len(f_proc_map) == 1 and len(c_proc_map) == 1:
                    c_proc = list(c_proc_map.values())[0]
                    self._add_mismatch("SYMBOL_NAME_MANGLING", Severity.WARNING, f"Mapped {name} to {c_proc.name}", name)
                else:
                    self._add_mismatch("UNMATCHED_PROCEDURE", Severity.WARNING,
            ...
            if f_proc.has_hidden_strlen:
                self._add_mismatch("HIDDEN_CHARACTER_LENGTH_ARG", Severity.ERROR,
                                   "Fortran CHARACTER(*) appends a hidden length argument that C does not see.",
                                   f_proc.name)

            # Param count mismatch
            if len(f_proc.params) != len(c_proc.params):
                self._add_mismatch("PARAMETER_COUNT_MISMATCH", Severity.ERROR,
            ...
            for i, (f_param_name, f_type) in enumerate(f_proc.params):
                if i < len(c_proc.params):
                    ...
                    if f_param_name != c_param_name:
                        self._add_mismatch("PARAM_ORDER", Severity.ERROR, f"Parameter name swap: {f_param_name} vs {c_param_name}", f_proc.name)
            ...
            if (f_proc.return_type is None) != (c_proc.return_type is None):
                ...
                self._add_mismatch("RETURN_TYPE_MISMATCH", Severity.ERROR,
        ...
        for name, c_proc in c_proc_map.items():
            if name not in f_proc_map:
                ...
                self._add_mismatch("UNMATCHED_PROCEDURE", Severity.WARNING,
                                   ...
                                   c_proc.name, file_path=c_proc.source_file, line_number=c_proc.source_line)
